...
class EncoderLayer:
    def __init__(self, model_name="VietAI/viT5-base-vietnews-summarization"):
        # Tải mô hình và tokenizer của VietAI/viT5-base-vietnews-summarization
        self.tokenizer = T5Tokenizer.from_pretrained(model_name)
        self.model = T5ForConditionalGeneration.from_pretrained(model_name)
        self.d_model = self.model.config.d_model  # 768 cho viT5-base
    ...
    def custom_encoder_layer(self):
        # 3️b. Mô phỏng Encoder Layer thủ công (cho viT5-base)
        multi_head_attention = nn.MultiheadAttention(embed_dim=self.d_model, num_heads=12, batch_first=True)  # 12 heads cho viT5-base
        ffn = nn.Sequential(
