...
class DecoderLayer:
    ...
    def load_encoder_outputs(self, encoder_output_path, attention_mask_path):
        ...
        self.encoder_outputs = torch.load(encoder_output_path)  # BaseModelOutput
        self.attention_mask = torch.load(attention_mask_path)

        # Kiểm tra dữ liệu
        if not isinstance(self.encoder_outputs, BaseModelOutput):
            raise ValueError(" Encoder outputs không đúng định dạng BaseModelOutput.")

        print("\n Đã tải Encoder Outputs (BaseModelOutput) và Attention Mask.")
        print(" Hidden States:", self.encoder_outputs.last_hidden_state[0, :7])
    ...
    def decoding_default(self, save_path="../object/decoder_outputs_default.pt"):
        ...
        with torch.no_grad():
            generated_ids = self.model.generate(
                ...
                encoder_outputs=self.encoder_outputs,  # Sử dụng đúng dạng BaseModelOutput
    ...
    def decoding_custom(self, save_path="../object/decoder_outputs_custom.pt"):
        ...
        with torch.no_grad():
            ...
            self.decoder_output_manual = decoder_layer(
                decoder_embeddings, 
                self.encoder_outputs.last_hidden_state
            )
        ...
        self.save_decoder_outputs(save_path, decoder_input_ids, custom=True)

    def save_decoder_outputs(self, path, decoder_input_ids, custom=False):
        data_to_save = {
            "last_hidden_state": self.decoder_output_manual if custom else self.encoder_outputs.last_hidden_state,
            "attention_mask": self.attention_mask,
