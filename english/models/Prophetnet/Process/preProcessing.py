...
# Load model và tokenizer cho ProphetNet
...
for i, paragraph in enumerate(cleaned_paragraphs):
    ...
    clean_tokens = [token.replace('▁', '').replace('</s>', '') for token in tokens]  # ProphetNet dùng ký tự '▁' thay cho 'Ġ'
    print(f"2. Đoạn {i+1}: Số lượng token: {len(clean_tokens)}")
    print(f"   Tokenized (Đã làm sạch): {clean_tokens}\n\n")

# Tạo Tensor đầu vào với max_length phù hợp với ProphetNet
